    ...
    await withinSandbox(async (sandbox) => {
      ...
      expect(result).toStrictEqual({
        ...
        failures: [
          {
            message:
              'Invalid `package.json`: Missing `version`; Missing `packageManager`; Missing `engines`; Missing `exports`; Missing `main`; Missing `module`; Missing `types`; Missing `files`; Missing `scripts`; Missing `devDependencies`; Missing `repository`; Missing `lavamoat`.',
          },
        ],
      });
    });
